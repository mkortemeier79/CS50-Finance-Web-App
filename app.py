...
def index():
    ...
    summaries = db.execute("""SELECT symbol, company, sum(shares) as sum_of_shares
                              FROM transactions
                              WHERE user_id = ?
                              GROUP BY user_id, symbol, company
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions(user_id, symbol, company, shares, price) VALUES(?, ?, ?, ?, ?);",
                   session["user_id"], symbol, query["name"], shares, query["price"])
...
def register():
    ...
    if request.method == "POST":
        ...
        if password != confirmation:
            return apology("passwords don't matched")
